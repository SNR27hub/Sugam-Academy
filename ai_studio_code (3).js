...
function initBannerSlider(banners) {
    ...
    const updateSliderAndButton = () => {
        ...
        // FIXED: Correctly set the background image from the banners array
        if (banners[currentBannerIndex] && banners[currentBannerIndex].imageUrl) {
    ...
    function dragStart(event) { isDragging = true; startPos = getPositionX(event); bannerSlider.style.transition = 'none'; bannerSlider.classList.add('grabbing'); clearInterval(bannerInterval); }
    function dragMove(event) { if (isDragging) { const currentPosition = getPositionX(event); const sliderWidth = bannerSlider.clientWidth; currentTranslate = prevTranslate + ((currentPosition - startPos) / sliderWidth) * 100; bannerSlider.style.transform = `translateX(${currentTranslate}%)`; } }
    function dragEnd() { if (!isDragging) return; isDragging = false; bannerSlider.classList.remove('grabbing'); const movedBy = currentTranslate - prevTranslate; if (movedBy < -10 && currentBannerIndex < slides.length - 1) currentBannerIndex++; if (movedBy > 10 && currentBannerIndex > 0) currentBannerIndex--; updateSliderAndButton(); }
...
function renderChatMessage(sender, text, timestamp) { const msgEl = document.createElement('div'); msgEl.className = `chat-message ${sender}`; msgEl.innerHTML = `<div>${text}</div><div class="chat-message-time">${new Date(timestamp).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</div>`; chatMessagesContainer.appendChild(msgEl); }
...
// --- FIXED: Instagram Popup with "Don't Show Again" logic ---
function showInstagramPopup() {
    const today = new Date().toDateString(); // Gets date like "Mon Sep 27 2023"
    const lastShown = localStorage.getItem('instagramPopupLastShown');

    if (lastShown === today) {
        return; // Don't show if it was already hidden today
    }
...
function closeInstagramPopup(isAutoClose = false) {
    ...
    if (dontShowAgainCheckbox.checked) {
        ...
        localStorage.setItem('instagramPopupLastShown', today);
    }

    popupContainer.style.animation = 'fadeOut 0.5s forwards';
    setTimeout(() => {
        instagramPopupModal.style.display = 'none';
        popupContainer.style.animation = ''; // Reset animation for next time
    }, 500);
...
searchInput.addEventListener('input', (e) => { const searchTerm = e.target.value.toLowerCase().trim(); const contentCards = document.querySelectorAll('#content .content-card'); contentCards.forEach(card => { const title = card.dataset.title || ''; if (title.includes(searchTerm)) { card.style.display = 'flex'; } else { card.style.display = 'none'; } }); });