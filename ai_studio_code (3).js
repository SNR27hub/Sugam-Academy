...
// This runs immediately to check if the site is under maintenance
async function checkMaintenanceMode() {
    try {
        ...
        if (maintenanceSnap.val() === true) {
            ...
            // Stop further script execution if in maintenance mode
            return true; 
        }
    } catch (error) {
...
let allContentData = []; // For search functionality
...
(async () => {
    ...
    if (isMaintenance) return; // Stop if in maintenance mode
...
function initializeEventListeners() {
    ...
    tabLinks.forEach(link => {
        link.addEventListener('click', () => handleTabChange(link.getAttribute('data-tab')));
    });
    ...
    adminNavLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault();
            handleAdminNav(link.getAttribute('data-section'));
        });
    });
    ...
    instagramPopupModal.querySelector('.follow-btn').addEventListener('click', () => {
...
function loadContent(isLoggedIn, isPremium, purchasedVideos = {}, favorites = {}, category) {
    ...
    contentRef.orderByChild('category').equalTo(category).on('value', snapshot => {
        ...
        handleSearch(); // Apply search filter after loading
    });
}
...
function openContentInNewTab(url) {
    if (!url) {
        return alert("Content link is not available.");
    }
...
async function openQuizModal(quizId) {
    ...
    quizOptions.innerHTML = currentQuizData.options.map(option =>
        `<label class="quiz-option"><input type="radio" name="quizOption" value="${option}">${option}</label>`
    ).join('');
...
function handleSubmitQuiz() {
    ...
    closeQuizFeedbackBtn.style.display = 'block';
...
// ... Other utility functions like payment submission, toggling favorites, etc. remain largely the same
...
async function renderFooter() {
    ...
    footerAboutUsText.textContent = settings.aboutUsText || 'Your trusted source for quality education.';
...
//... (Functions for user modals like myPurchases, myFavorites, top scorer, instagram popup etc.)
async function openMyPurchasesModal() {
    ...
    db.ref('payments').orderByChild('userId').equalTo(user.uid).on('value', snapshot => {
        ...
        snapshot.forEach(child => {
            const p = child.val();
            const row = purchasesTableBody.insertRow(0); // Prepend
...
async function openMyFavoritesModal() {
    ...
    favoritesContentGrid.innerHTML = '<p>Loading...</p>';
    ...
    if (favoriteIds.length === 0) {
        return favoritesContentGrid.innerHTML = '<p>No favorites yet.</p>';
    }

    const favoritePromises = favoriteIds.map(id => db.ref(`content/${id}`).once('value'));
    const favoriteSnaps = await Promise.all(favoritePromises);
    
    favoritesContentGrid.innerHTML = '';
    const fragment = document.createDocumentFragment();
    favoriteSnaps.forEach(snap => {
        if (snap.exists()) {
            fragment.appendChild(createContentCard(snap.val(), snap.key, true, userData.premium, userData.purchasedVideos, userData.favorites));
        }
    });
    favoritesContentGrid.appendChild(fragment);
}
...
function closeInstagramPopup() {
    if (instagramPopupModal.style.display === 'none') return;
    const container = instagramPopupModal.querySelector('.popup-container');
...
async function showAdminPanel() {
    ...
    // Initialize all admin listeners once
    initializeAdminEventListeners();
    
    // Load initial data
    handleAdminNav('dashboard'); // Load dashboard by default
}

function initializeAdminEventListeners() {
    // Only run this once to avoid duplicate listeners
    if (adminPanel.dataset.listenersInitialized) return;
    adminPanel.dataset.listenersInitialized = 'true';
    ...
    bannerForm.addEventListener('submit', handleBannerForm);

    addToolBtn.addEventListener('click', () => { toolForm.reset(); toolForm.dataset.editId = ''; toolFormContainer.style.display = 'block'; });
    cancelTool.addEventListener('click', () => toolFormContainer.style.display = 'none');
    toolForm.addEventListener('submit', handleToolForm);
    ...
    contentForm.addEventListener('submit', handleContentForm);
    
    notificationForm.addEventListener('submit', handleNotificationForm);
    deleteAllNotificationsBtn.addEventListener('click', () => { if (confirm('Delete ALL notifications?')) db.ref('notifications').remove(); });
    ...
    teacherForm.addEventListener('submit', handleTeacherForm);
    ...
    generalSettingsForm.addEventListener('submit', handleSettingsForm);
}

function handleAdminNav(section) {
    adminNavLinks.forEach(l => l.classList.remove('active'));
    document.querySelector(`.admin-nav a[data-section="${section}"]`).classList.add('active');
    adminSections.forEach(s => s.classList.remove('active'));
    
    const sectionId = section === 'tools' ? 'toolsSectionAdmin' : `${section}Section`;
    ...
    switch(section) {
        ...
        case 'tools': loadAdminTools(); break;
        case 'users': loadUsers(); break;
        case 'content': loadAdminContent(); break;
        case 'payments': loadPayments(); break;
        case 'notifications': loadAdminNotifications(); break;
        case 'chats': loadAdminChatList(); break;
        case 'teachers': loadAdminTeachers(); break;
        case 'plans': loadAdminPlans(); break;
        case 'settings': loadAdminSettings(); break;
    }
}
//... (All other admin functions: loadDashboardStats, loadUsers, loadPayments, etc.)
// ... (These functions are quite long, but their logic doesn't need to change much, just ensure they are called correctly by handleAdminNav)

// Load Dashboard
function loadDashboardStats() {
    db.ref('users').once('value').then(snap => {
        totalStudents.textContent = snap.numChildren();
        premiumUsers.textContent = Object.values(snap.val() || {}).filter(u => u.premium).length;
    });
    db.ref('teachers').once('value').then(snap => totalTeachers.textContent = snap.numChildren());
    db.ref('payments').once('value').then(snap => {
        pendingPayments.textContent = Object.values(snap.val() || {}).filter(p => p.status === 'pending').length;
    });
}
// Load Admin Banners
function loadAdminBanners() {
    db.ref('banners').on('value', snapshot => {
        ...
        snapshot.forEach(child => {
            ...
            row.innerHTML = `
                <td>${b.title || 'N/A'}</td>
                <td><img src="${b.imageUrl}" style="width: 100px;"></td>
...
function editBanner(id) {
    db.ref('banners/' + id).once('value').then(snap => {
        ...
        bannerFormContainer.scrollIntoView({ behavior: 'smooth', block: 'center' });
    });
}
    ...
    db.ref('tools').on('value', snapshot => {
        ...
        snapshot.forEach(child => {
            ...
            row.innerHTML = `
                ...
                <td>${t.imageUrl ? `<img src="${t.imageUrl}" style="width: 50px;">` : 'None'}</td>
                <td>
...
function editTool(id) {
    db.ref('tools/' + id).once('value').then(snap => {
        ...
        document.getElementById('toolImageUrl').value = t.imageUrl || '';
        toolForm.dataset.editId = id;
        toolFormContainer.style.display = 'block';
        toolFormContainer.scrollIntoView({ behavior: 'smooth', block: 'center' });
    });
}
...
function loadAdminContent() {
    db.ref('content').on('value', snapshot => {
        adminContentGrid.innerHTML = ''; if (!snapshot.exists()) return;
        snapshot.forEach(child => {
            const content = child.val();
            const card = document.createElement('div');
            card.className = 'content-card';
            const thumbHtml = content.thumbnailUrl ? `<img src="${content.thumbnailUrl}" alt="Thumb">` : `<i class="fas fa-${content.type === 'video' ? 'video' : content.type === 'note' ? 'file-pdf' : 'brain'}"></i>`;
            card.innerHTML = `
                <div class="content-thumb">${thumbHtml}</div>
                <div class="content-info">
                    <h3>${content.title || 'N/A'} ${content.access !== 'free' ? `<span class="premium-badge">${content.access}</span>` : ''}</h3>
...
function handleContentForm(e) {
    ...
    ref.set(data).then(() => contentFormContainer.style.display = 'none');
}
function editContent(id) {
    db.ref('content/' + id).once('value').then(snap => {
        const c = snap.val();
        document.getElementById('contentTitle').value = c.title || '';
        document.getElementById('contentThumbnailUrl').value = c.thumbnailUrl || '';
        document.getElementById('contentCategory').value = c.category || '+2';
        document.getElementById('contentType').value = c.type || 'video';
        document.getElementById('contentUrl').value = c.url || '';
        document.getElementById('contentAccess').value = c.access || 'free';
        document.getElementById('contentPrice').value = c.price || '';
        
        contentPriceContainer.style.display = (c.access !== 'free') ? 'block' : 'none';
        quizFields.style.display = c.type === 'quiz' ? 'block' : 'none';
        if (c.type === 'quiz') {
            document.getElementById('quizQuestion').value = c.question || '';
            document.getElementById('quizOption1').value = c.options?.[0] || '';
            document.getElementById('quizOption2').value = c.options?.[1] || '';
            document.getElementById('quizOption3').value = c.options?.[2] || '';
            document.getElementById('quizOption4').value = c.options?.[3] || '';
            document.getElementById('quizCorrectAnswer').value = c.correctAnswer || '';
        }
...
// ... The rest of the admin functions (payments, notifications, teachers, plans, settings, users, chat) can remain as they are, they don't need significant changes based on the request.
// Just ensure they are loaded correctly.
// --- Google Translate ---
function googleTranslateElementInit() {
    new google.translate.TranslateElement({ pageLanguage: 'en', includedLanguages: 'en,or,hi,bn,te,ta', autoDisplay: false }, 'google_translate_element');
}

// All other functions like loadUsers, loadPayments, handleSettingsForm etc. go here.
// They are mostly unchanged from your original file, so I'm omitting them for brevity but they should be included in the final file.
// For example, loadUsers:
function loadUsers() {
...
function loadPayments() {
    db.ref('payments').on('value', snapshot => {
        ...
        snapshot.forEach(child => {
            ...
            paymentPromises.push(db.ref('users/' + payment.userId).once('value').then(userSnap => ({ payment, user: userSnap.val() })));
        });
        Promise.all(paymentPromises).then(results => {
            results.forEach(({ payment, user }) => {
                const row = paymentsTableBody.insertRow();
                let actions = `<span class="status-badge status-${payment.status}">${payment.status}</span>`;
                if (payment.status === 'pending') {
                    actions = `
                        <button class="btn btn-primary" onclick="verifyPayment('${payment.key}', '${payment.userId}', '${payment.type}', '${payment.contentId}')">Verify</button>
                        <button class="btn btn-outline" onclick="rejectPayment('${payment.key}')">Reject</button>
                    `;
                }
                row.innerHTML = `
...
function verifyPayment(paymentId, userId, type, contentId) {
    db.ref('payments/' + paymentId).update({ status: 'verified' }).then(() => {
        if (type === 'premium') {
            db.ref('users/' + userId).update({ premium: true });
        } else if (type === 'video') {
            db.ref(`users/${userId}/purchasedVideos/${contentId}`).set(true);
        } else if (type === 'verifyBadge') {
            db.ref('users/' + userId).update({ isVerified: true });
        }
...
function rejectPayment(paymentId) {
    if (confirm('Reject this payment?')) db.ref('payments/' + paymentId).update({ status: 'rejected' });
}

// Load Admin Teachers
function loadAdminTeachers() {
    db.ref('teachers').on('value', snapshot => {
        teachersTableBody.innerHTML = ''; if (!snapshot.exists()) return;
        snapshot.forEach(child => {
            const t = child.val();
            const row = teachersTableBody.insertRow();
            row.innerHTML = `
                <td>${t.name || 'N/A'} ${t.verified ? `<img src="${VERIFIED_BADGE_IMG_URL}" alt="Verified" class="verified-badge-img">` : ''}</td>
                <td>${t.subject || 'N/A'}</td>
                <td>
...
function handleTeacherForm(e) { e.preventDefault(); const name = document.getElementById('teacherName').value.trim(); const subject = document.getElementById('teacherSubject').value.trim(); if (!name || !subject) return alert('Name and Subject are required.'); const data = { name, subject, experience: document.getElementById('teacherExperience').value.trim() || null, photoUrls: document.getElementById('teacherPhotoUrls').value.trim() || null, verified: document.getElementById('teacherVerified').checked }; const editId = teacherForm.dataset.editId; const ref = editId ? db.ref('teachers/' + editId) : db.ref('teachers').push(); ref.set(data).then(() => teacherFormContainer.style.display = 'none'); }
...
// The rest of the Admin, Chat, and other functions can be pasted here without major changes.
async function handleSettingsForm(e) {
    ...
    if (settingsData.maintenanceModeEnabled) {
        // If maintenance is turned on, show overlay immediately for the admin too
        document.getElementById('maintenanceOverlay').style.display = 'flex';
    }
}
async function loadAdminSettings() {
    const settings = (await db.ref('settings').once('value')).val() || {};
    maintenanceModeToggle.checked = settings.maintenanceModeEnabled || false;
    adminVerifyBadgePrice.value = settings.adminVerifyBadgePrice || '';
    toolsSectionTitleInput.value = settings.toolsSectionTitle || '';
...
// All other functions like chat, plans etc. are also here
...
function loadAdminNotifications(){db.ref("notifications").on("value",e=>{notificationsTableBody.innerHTML="",e.exists()&&e.forEach(e=>{const t=e.val(),o=notificationsTableBody.insertRow;o.innerHTML=`<td>${t.title||"N/A"}</td><td>${t.message||"N/A"}</td><td>${t.imageUrl?`<a href="${t.imageUrl}" target="_blank">View</a>`:"None"}</td><td>${new Date(t.date).toLocaleDateString()}</td>`})})}
function handlePlanForm(e){e.preventDefault();const t=document.getElementById("planName").value.trim(),a=parseFloat(document.getElementById("planPrice").value);if(!t||isNaN(a)||a<=0)return alert("Name and valid price are required.");const n={name:t,price:a,description:document.getElementById("planDescription").value.trim()||null},o=planForm.dataset.editId;o?db.ref("plans/"+o).update(n).then(()=>alert("Plan updated!")):db.ref("plans").push(n).then(()=>alert("Plan added!")),planFormContainer.style.display="none"}
function editPlan(e){db.ref("plans/"+e).once("value").then(t=>{const a=t.val();document.getElementById("planName").value=a.name||"",document.getElementById("planPrice").value=a.price||"",document.getElementById("planDescription").value=a.description||"",planForm.dataset.editId=e,planFormContainer.style.display="block",planFormContainer.scrollIntoView({behavior:"smooth",block:"center"})})}
function deletePlan(e){confirm("Delete this plan?")&&db.ref("plans/"+e).remove().then(()=>alert("Plan deleted!"))}
function loadAdminPlans(){db.ref("plans").on("value",e=>{plansTableBody.innerHTML="",e.exists()&&e.forEach(e=>{const t=e.val(),a=plansTableBody.insertRow();a.innerHTML=`<td>${t.name||"N/A"}</td><td>₹${t.price||"0"}</td><td>${t.description||"N/A"}</td><td><button class="btn btn-primary" onclick="editPlan('${e.key}')">Edit</button><button class="btn btn-outline" onclick="deletePlan('${e.key}')">Delete</button></td>`})})}
function initializeUserChat(){const e=auth.currentUser;e&&db.ref(`chats/${e.uid}/messages`).orderByChild("timestamp").on("value",e=>{chatMessagesContainer.innerHTML="",e.exists()?e.forEach(e=>renderChatMessage(e.val().sender,e.val().text,e.val().timestamp)):chatMessagesContainer.innerHTML='<p style="text-align:center; color:#94a3b8;">Start a conversation!</p>',chatMessagesContainer.scrollTop=chatMessagesContainer.scrollHeight})}
function handleChatSubmit(e){e.preventDefault();const t=auth.currentUser,a=chatMessageInput.value.trim();if(t&&a){const e=firebase.database.ServerValue.TIMESTAMP;db.ref(`chats/${t.uid}/messages`).push({sender:"user",text:a,timestamp:e}),db.ref(`chats/${t.uid}/meta`).update({lastMessage:a,lastTimestamp:e,userName:t.displayName||t.email,hasUnreadAdminMessage:!0}),chatMessageInput.value=""}}
function renderChatMessage(e,t,a,n=chatMessagesContainer){const s=document.createElement("div");s.className=`chat-message ${e}`,s.innerHTML=`<div>${t}</div><div class="chat-message-time">${new Date(a).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</div>`,n.appendChild(s)}
