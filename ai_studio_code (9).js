...
// --- Auth State Change Handler (The Core Logic) ---
auth.onAuthStateChanged(async user => {
    const isMaintenance = await checkMaintenanceMode();
    if (isMaintenance && (!user || user.email !== 'sugamacadamy@gmail.com')) {
        return; // Stop everything if in maintenance mode for non-admins
    }
    ...
    if (user) {
        ...
        if (user.email === 'sugamacadamy@gmail.com' && isAdmin) {
            showAdminPanel();
        } else if (user.email === 'sugamacadamy@gmail.com' && !isAdmin) {
            alert("Admin Login Failed! Your UID is not registered in the database's 'admins' section. Please contact the developer to set it up.");
            auth.signOut(); // Log out the failed admin attempt
        } else {
            ...
            db.ref('users/' + user.uid).on('value', snap => {
                ...
                if (userData) {
                    ...
                    // This can happen if user exists in Auth but not in DB
                    console.log("User data not found in database, logging out.");
                    auth.signOut();
                }
            }, error => {
                // THIS IS THE CRITICAL PART - NO MORE AUTO-LOGOUT
                console.error("Firebase read failed for user data:", error);
                alert("Could not load your profile data due to a permission error. Please try again later.");
                // We DON'T sign out here, to prevent the loop.
            });
        }
    } else {
...
// --- All other functions go below this line ---
...
(async () => {
    ...
    // The auth handler is now at the top level
})();
...
function handleBannerForm(e){e.preventDefault();const t=document.getElementById("bannerTitle").value.trim(),o=document.getElementById("bannerImageUrl").value.trim();if(!t||!o)return alert("Title and Image URL are required.");const n={title:t,imageUrl:o,buttonText:document.getElementById("bannerButtonText").value.trim()||null,buttonUrl:document.getElementById("bannerButtonUrl").value.trim()||null,startDate:document.getElementById("bannerStartDate").value||null,endDate:document.getElementById("bannerEndDate").value||null},a=bannerForm.dataset.editId,i=a?db.ref("banners/"+a):db.ref("banners").push();i.set(n).then(()=>{alert("Banner saved successfully!"),bannerFormContainer.style.display="none"}).catch(e=>alert("Error saving banner: "+e.message))}
function handleToolForm(e){e.preventDefault();const t=document.getElementById("toolTitle").value.trim(),o=document.getElementById("toolLink").value.trim();if(!t||!o)return alert("Title and Link are required.");const n={title:t,link:o,imageUrl:document.getElementById("toolImageUrl").value.trim()||null},a=toolForm.dataset.editId,i=a?db.ref("tools/"+a):db.ref("tools").push();i.set(n).then(()=>{alert("Tool saved successfully!"),toolFormContainer.style.display="none"}).catch(e=>alert("Error saving tool: "+e.message))}
function handleContentForm(e){e.preventDefault();const t=document.getElementById("contentTitle").value.trim(),o=document.getElementById("contentCategory").value;if(!t||!o)return alert("Title and Category are required.");const n={title:t,category:o,thumbnailUrl:document.getElementById("contentThumbnailUrl").value.trim()||null,type:document.getElementById("contentType").value,url:document.getElementById("contentUrl").value.trim()||null,access:document.getElementById("contentAccess").value,price:parseFloat(document.getElementById("contentPrice").value)||null};"quiz"===n.type&&(n.question=document.getElementById("quizQuestion").value.trim(),n.options=[document.getElementById("quizOption1").value.trim(),document.getElementById("quizOption2").value.trim(),document.getElementById("quizOption3").value.trim(),document.getElementById("quizOption4").value.trim()].filter(Boolean),n.correctAnswer=document.getElementById("quizCorrectAnswer").value.trim());const a=contentForm.dataset.editId,i=a?db.ref("content/"+a):db.ref("content").push();i.set(n).then(()=>{alert("Content saved successfully!"),contentFormContainer.style.display="none"}).catch(e=>alert("Error saving content: "+e.message))}
function handleTeacherForm(e){e.preventDefault();const t=document.getElementById("teacherName").value.trim(),o=document.getElementById("teacherSubject").value.trim();if(!t||!o)return alert("Name and Subject are required.");const n={name:t,subject:o,experience:document.getElementById("teacherExperience").value.trim()||null,photoUrls:document.getElementById("teacherPhotoUrls").value.trim()||null,verified:document.getElementById("teacherVerified").checked},a=teacherForm.dataset.editId,i=a?db.ref("teachers/"+a):db.ref("teachers").push();i.set(n).then(()=>{alert("Teacher saved successfully!"),teacherFormContainer.style.display="none"}).catch(e=>alert("Error saving teacher: "+e.message))}
function handlePlanForm(e){e.preventDefault();const t=document.getElementById("planName").value.trim(),o=parseFloat(document.getElementById("planPrice").value);if(!t||isNaN(o)||o<=0)return alert("Name and valid price are required.");const n={name:t,price:o,description:document.getElementById("planDescription").value.trim()||null},a=planForm.dataset.editId,i=a?db.ref("plans/"+a):db.ref("plans").push();i.set(n).then(()=>{alert("Plan saved successfully!"),planFormContainer.style.display="none"}).catch(e=>alert("Error saving plan: "+e.message))}
async function handleSettingsForm(e){e.preventDefault();const t={maintenanceModeEnabled:maintenanceModeToggle.checked,adminVerifyBadgePrice:parseFloat(adminVerifyBadgePrice.value)||null,toolsSectionTitle:toolsSectionTitleInput.value.trim()||null,displayTotalUsersCount:displayTotalUsersCountInput.value.trim()||null,displayUserNames:displayUserNamesInput.value.split(",").map(e=>e.trim()).filter(Boolean),aboutUsText:settingsAboutUsText.value.trim()||null,contact:{sugamEmail:settingsSugamEmail.value.trim()||null,developerEmail:settingsDeveloperEmail.value.trim()||null},socialLinks:{youtube:settingsYoutubeLink.value.trim()||null,instagram:settingsInstagramLink.value.trim()||null,facebook:settingsFacebookLink.value.trim()||null,twitter:settingsTwitterLink.value.trim()||null}};try{await db.ref("settings").update(t),alert("Settings saved successfully!"),t.maintenanceModeEnabled&&"sugamacadamy@gmail.com"!==auth.currentUser.email&&(document.getElementById("maintenanceOverlay").style.display="flex")}catch(e){alert("Error saving settings: "+e.message)}}
function loadUsers(){db.ref("users").on("value",e=>{usersTableBody.innerHTML="",e.exists()?e.forEach(e=>{const t=e.val(),o=usersTableBody.insertRow();o.innerHTML=`\n                <td>${t.name||"N/A"} ${t.isVerified?`<img src="${VERIFIED_BADGE_IMG_URL}" alt="Verified" class="verified-badge-img">`:""}</td>\n                <td>${t.email||"N/A"}</td>\n                <td>${t.premium?'<span class="premium-badge">Premium</span>':"Free"}</td>\n                <td>\n                    <button class="btn btn-outline" onclick="blockUser('${e.key}')">Block</button>\n                </td>\n            `}):usersTableBody.innerHTML='<tr><td colspan="4" class="text-center">No users found. (Check Firebase Rules)</td></tr>'},e=>{console.error("Firebase read failed for users:",e),usersTableBody.innerHTML='<tr><td colspan="4" class="text-center">Error loading users. Check Firebase Rules and console for details.</td></tr>'})}