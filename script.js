...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (settings.maintenanceMode) {
        ...
        console.log("Maintenance mode is active. Displaying maintenance screen.");
        return;
    }
    ...
    auth.onAuthStateChanged(user => {
        if (user && user.email === 'sugamacadamy@gmail.com') {
            ...
            console.log("No user authenticated. Updating UI for logged out state.");
            updateUI(null);
        }
    });
    ...
    renderFooter();
    // setTimeout(showInstagramPopup, 2000); // Instagram popup will be re-enabled after fixing its click issues
});
...
function updateUI(userData) {
    console.log("updateUI called with userData:", userData);
    if (userData) {
        ...
        profileDropdownMenu.style.display = 'none';
        currentUserView = '+2'; // Default to +2 for logged out users
        categorySelector.querySelectorAll('button').forEach(btn => {
            btn.classList.toggle('active', btn.dataset.category === currentUserView);
        });
    ...
    db.ref('banners').on('value', snapshot => {
        ...
        if (banners.length > 0) {
            ...
            if (banners.length === 1 && !isFirstImageLoaded) {
                console.log("Only one banner or image cached, force initializing slider.");
                setTimeout(() => initBannerSlider(banners), 100);
            } else if (!isFirstImageLoaded && banners.length > 1) {
                // Fallback for multiple banners if onload somehow doesn't trigger
                console.log("Multiple banners, but first image onload not triggered, fallback initializing slider.");
                setTimeout(() => initBannerSlider(banners), 1000); // A bit longer fallback
            }
...
function initBannerSlider(banners) {
    ...
    if (slides.length === 0) {
        console.log("initBannerSlider called, but no slides found. Hiding banner container.");
    ...
    console.log("Initializing banner slider with", slides.length, "slides. Current index:", currentBannerIndex);

    const initialImageUrl = banners[0]?.imageUrl;
    if (heroDynamicBg) {
        heroDynamicBg.style.backgroundImage = initialImageUrl ? `url('${initialImageUrl}')` : 'none';
        console.log("Setting hero background to:", initialImageUrl);
    }
    ...
    const updateSliderAndButton = () => {
        ...
        if (currentBanner?.buttonText && currentBanner?.buttonUrl) {
            ...
            console.log("Banner button created:", currentBanner.buttonText);
        } else {
            currentBannerButtonWrapper.style.display = 'none';
            console.log("No banner button for current slide.");
        }
        resetAutoSlide();
    };
    ...
    const nextSlide = () => {
        ...
        console.log("Next slide triggered. New index:", currentBannerIndex);
    ...
    const startAutoSlide = () => {
        ...
        if (slides.length > 1) {
            ...
            console.log("Only one slide, auto-slide not needed.");
        }
    };
    ...
    function dragStart(event) {
        ...
        console.log("Drag started. Prev translate:", prevTranslate);
    }
    ...
    function dragEnd() {
        ...
        console.log("Drag ended. New index after drag:", currentBannerIndex);
    ...
    prevBtn.addEventListener('click', () => {
        ...
        console.log("Prev slide triggered. New index:", currentBannerIndex);
        updateSliderAndButton();
    });
...
function loadTools() {
    console.log("Attempting to load tools...");
    db.ref('tools').on('value', snapshot => {
        ...
        if (toolsData && toolsData.items) {
            ...
            console.log(`Loaded tools section title: ${toolsSectionTitle.textContent}`);
            const items = Object.values(toolsData.items);
            console.log(`Found ${items.length} tool items.`);
            items.forEach(tool => {
            ...
            console.log("No tools data found in Firebase.");
        }
    });
}

function loadContent(isLoggedIn, isPremium, purchasedVideos = {}, favorites = {}, category) {
    console.log(`loadContent called. Category: ${category}, LoggedIn: ${isLoggedIn}, Premium: ${isPremium}`);
    document.getElementById('content').style.display = 'block'; // Ensure content section is visible
    db.ref('content').orderByChild('category').equalTo(category).on('value', snapshot => {
        ...
        if (snapshot.exists()) {
            ...
            console.log(`Fetched ${allContentData.length} content items for category ${category}.`);
        } else {
            console.log(`No content found for category ${category} in Firebase.`);
        }
...
function renderContent(contentArray, isLoggedIn, isPremium, purchasedVideos, favorites) {
    console.log(`renderContent called with ${contentArray.length} items. Rendering...`);
    ...
    console.log(`Content rendering complete. Videos: ${hasVideos}, Notes: ${hasNotes}, Quizzes: ${hasQuizzes}`);
}

function createContentCard(content, contentId, isLoggedIn, isPremium, purchasedVideos, favorites) {
    // console.log("Creating card for:", content.title, "Type:", content.type, "Access:", content.access); // Log each card creation
    const card = document.createElement('div');
    card.className = 'content-card';
    card.dataset.title = content.title ? content.title.toLowerCase() : '';
    ...
    if (content.access === 'free') {
        ...
        // Fallback for content with no specified access or unknown access type
        actionButton = `<button class="btn btn-outline" disabled>Access Restricted</button>`;
        console.warn(`Content "${content.title}" (ID: ${contentId}) has undefined or unknown 'access' type: ${content.access}.`);
    }
...
function loadTeachers() {
    ...
    db.ref('teachers').on('value', snapshot => {
        ...
        if (!snapshot.exists()) {
            ...
            console.log("No teachers data found in Firebase.");
            return;
        }
        const teachers = [];
            ...
            if (photos.length > 1) {
                teacherCard.addEventListener('mouseenter', () => {
                    intervalId = setInterval(() => {
                        ...
                        if (!slider) { clearInterval(intervalId); return; } // Safety check
                        const slides = slider.querySelectorAll('.slide');
                        if (slides.length === 0) { clearInterval(intervalId); return; } // Safety check
...
dropdownLogoutBtn.addEventListener('click', () => { auth.signOut(); console.log("User logged out."); });
...
function openPremiumModal() {
    ...
    db.ref('plans').once('value').then(snap => {
        ...
        console.log("Premium plan amount set to:", displayPremiumAmount.textContent);
    });
}

function openContentDisplayModal(url, type, title) {
    console.log(`Opening content display modal for: ${title}, Type: ${type}, URL: ${url}`);
    if (!url) {
        ...
        console.warn(`Content "${title}" has no URL provided.`);
        return;
    }
    // Changed to open in new tab directly as per previous instruction and common practice for external content
...
async function openQuizModal(quizId) {
    console.log("Opening Quiz modal for ID:", quizId);
    ...
    if (!currentQuizData || !currentQuizData.question) {
        alert('Quiz not configured or questions missing.');
        console.error("Quiz data not found or question missing for ID:", quizId);
        return;
    }
...
submitQuizAnswerBtn.addEventListener('click', () => {
    ...
    console.log("Quiz answer submitted:", selectedOption.value);
    ...
    if (selectedOption.value === currentQuizData.correctAnswer) {
        ...
        if(user) db.ref(`users/${user.uid}/name`).once('value').then(nameSnap => {
            const userName = nameSnap.val() || 'Anonymous';
            db.ref('topScorer').set({ userName: userName, quizTitle: currentQuizData.title || 'a quiz' });
            console.log(`Top scorer updated: ${userName} for quiz "${currentQuizData.title}".`);
        });
    } else {
...
closeQuizFeedbackBtn.addEventListener('click', () => { quizModal.style.display = 'none'; console.log("Quiz modal closed."); });
...
function toggleFavorite(contentId) {
    ...
    favoriteRef.once('value', snapshot => {
        if (snapshot.exists()) {
            favoriteRef.remove();
            console.log(`Removed ${contentId} from favorites.`);
        }
        else {
            favoriteRef.set(true);
            console.log(`Added ${contentId} to favorites.`);
        }
    });
}
...
function submitPaymentDetails(paymentType) {
    ...
    .then(() => {
        ...
        itemUTRNumber.value = '';
        console.log(`Payment details submitted for ${paymentType}:`, { utr: utrNumber, amount: amount, title: title });
    })
    .catch(error => {
...
function openMyPurchasesModal() {
    const user = auth.currentUser;
    if (!user) { console.log("Not logged in, cannot open purchases modal."); return; }
    console.log("Opening My Purchases modal for user:", user.uid);
    db.ref('payments').orderByChild('userId').equalTo(user.uid).on('value', snapshot => {
        purchasesTableBody.innerHTML = '';
        if (!snapshot.exists()) {
            console.log("No purchases found for user.");
            purchasesTableBody.innerHTML = '<tr><td colspan="4" class="text-center">No purchases yet.</td></tr>';
            return;
        }
        snapshot.forEach(child => {
        ...
        console.log(`Displayed ${snapshot.numChildren()} purchase records.`);
    }, (error) => {
...
async function openMyFavoritesModal() {
    const user = auth.currentUser;
    if (!user) { console.log("Not logged in, cannot open favorites modal."); return; }
    console.log("Opening My Favorites modal for user:", user.uid);
    ...
    if (favoriteIds.length === 0) {
        favoritesContentGrid.innerHTML = '<p>No favorites yet.</p>';
        console.log("No favorites found for user.");
        return;
    }
    console.log(`Found ${favoriteIds.length} favorite IDs.`);
    const favoriteSnaps = await Promise.all(favoriteIds.map(id => db.ref(`content/${id}`).once('value')));
    favoritesContentGrid.innerHTML = '';
    let loadedFavoritesCount = 0;
    favoriteSnaps.forEach(snap => {
        if (snap.exists()) {
            favoritesContentGrid.appendChild(createContentCard(snap.val(), snap.key, true, userData.premium, userData.purchasedVideos, userData.favorites));
            loadedFavoritesCount++;
        } else {
            console.warn(`Favorite content with ID ${snap.key} not found in 'content' database.`);
        }
    });
    console.log(`Displayed ${loadedFavoritesCount} favorite content cards.`);
}
...
function showToastNotification(notification) {
    ...
    console.log("Toast notification displayed:", notification.title);
}

// --- Chat Functionality ---
chatFab.addEventListener('click', () => { chatWidget.style.display = chatWidget.style.display === 'flex' ? 'none' : 'flex'; console.log("Chat FAB clicked. Widget visibility:", chatWidget.style.display); });
closeChatWidget.addEventListener('click', () => { chatWidget.style.display = 'none'; console.log("Chat widget closed."); });
function initializeUserChat() {
    const user = auth.currentUser;
    if (!user) { console.log("Chat: No user for chat initialization."); return; }
    ...
    db.ref(`chats/${user.uid}/messages`).orderByChild('timestamp').on('value', snapshot => {
        ...
        if (snapshot.exists()) {
            snapshot.forEach(child => renderChatMessage(child.val().sender, child.val().text, child.val().timestamp));
            console.log(`Chat: Rendered ${snapshot.numChildren()} messages.`);
        } else {
            chatMessagesContainer.innerHTML = '<p class="text-center" style="padding: 1rem; color: #94a3b8;">Start a conversation!</p>';
            console.log("Chat: No messages found, displaying placeholder.");
        }
...
chatInputForm.addEventListener('submit', (e) => {
    ...
    if (!user || !text) { console.warn("Chat: Message send failed. User or text missing."); return; }
    const timestamp = firebase.database.ServerValue.TIMESTAMP;
    db.ref(`chats/${user.uid}/messages`).push({ sender: 'user', text, timestamp })
    .then(() => console.log("Chat: Message sent by user."))
    .catch(error => console.error("Chat: Error sending message:", error));
...
async function renderFooter() {
    ...
    footerAboutUsText.textContent = settings.aboutUsText || 'Your premier destination for online learning.';
...
function loadTopScorer() {
    console.log("Loading top scorer...");
    db.ref('topScorer').on('value', snapshot => {
        if (snapshot.exists()) {
            ...
            topScorerTimeout = setTimeout(() => { topScorerBanner.style.opacity = 0; setTimeout(() => { topScorerBanner.style.display = 'none'; }, 500); }, 15000);
            console.log("Top scorer banner displayed:", data.userName, data.quizTitle);
        } else {
            topScorerBanner.style.display = 'none';
            console.log("No top scorer data found.");
        }
    });
}
async function loadDynamicUserDisplay() {
    console.log("Loading dynamic user display...");
    const settings = (await db.ref('settings').once('value')).val() || {};
    userNotificationCount.textContent = settings.displayTotalUsersCount || '0';
    const realUserNames = [];
    (await db.ref('users').limitToFirst(10).once('value')).forEach(child => realUserNames.push(child.val().name));
    ...
    if (displayedUserNames.length > 0) {
        ...
        const rotateName = () => {
            ...
            setTimeout(() => { userNotificationBar.classList.remove('show'); }, 7000); // Hide after 7 seconds
        };
        // Initial display
        rotateName();
        // Start interval for rotation
        userNotificationBarInterval = setInterval(rotateName, 20000); // Repeat every 20 seconds
        console.log(`Dynamic user display started with ${displayedUserNames.length} names.`);
    } else {
        userNotificationBar.style.display = 'none';
        console.log("No user names to display in dynamic bar.");
    }
}
...
function setupInstagramPopup() {
    ...
    if (closeIcon) closeIcon.addEventListener('click', () => { console.log("Instagram close icon clicked."); closeInstagramPopup(); });
    if (followBtn) followBtn.addEventListener('click', () => { console.log("Instagram follow button clicked."); followUser(); });
    if (closeBtn) closeBtn.addEventListener('click', () => { console.log("Instagram close button clicked."); closeInstagramPopup(); });
}

function showInstagramPopup() {
    console.log("Attempting to show Instagram popup...");
    ...
    popupAutoCloseTimer = setTimeout(() => {
        console.log("Instagram popup auto-closing.");
...
function closeInstagramPopup(isAutoClose = false) {
    ...
    if (dontShowAgainCheckbox && dontShowAgainCheckbox.checked) {
        ...
        console.log("Instagram popup will not show again today.");
    }
...
function createParticles() {
    ...
    if (!particlesContainer) { console.warn("Particles container not found."); return; }
...
// Share Button
function copyToClipboard(text) { navigator.clipboard.writeText(text).then(() => { alert('Link copied to clipboard!'); console.log("Link copied to clipboard."); }).catch(err => { console.error('Failed to copy: ', err); alert('Failed to copy link.'); }); }
shareButton.addEventListener('click', () => { const shareData = { title: 'Sugam Academy', text: 'Check out Sugam Academy - The best learning platform!', url: window.location.href }; if (navigator.share && navigator.canShare(shareData)) { navigator.share(shareData); console.log("Web Share API used."); } else { copyToClipboard(window.location.href); } });
...
searchInput.addEventListener('input', (e) => {
    ...
    console.log(`Search completed. Found ${foundCount} matching cards.`);
});
