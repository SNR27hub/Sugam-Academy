...
function initializeAdminEventListeners() {
    ...
    addBannerBtn.addEventListener('click', () => { bannerForm.reset(); bannerForm.dataset.editId = ''; bannerFormContainer.style.display = 'block'; bannerFormContainer.scrollIntoView({ behavior: 'smooth', block: 'center' }); });
    ...
    addToolBtn.addEventListener('click', () => { toolForm.reset(); toolForm.dataset.editId = ''; toolFormContainer.style.display = 'block'; toolFormContainer.scrollIntoView({ behavior: 'smooth', block: 'center' }); });
    ...
    addTeacherBtn.addEventListener('click', () => { teacherForm.reset(); teacherForm.dataset.editId = ''; teacherFormContainer.style.display = 'block'; teacherFormContainer.scrollIntoView({ behavior: 'smooth', block: 'center' }); });
    ...
    addPlanBtn.addEventListener('click', () => { planForm.reset(); planForm.dataset.editId = ''; planFormContainer.style.display = 'block'; planFormContainer.scrollIntoView({ behavior: 'smooth', block: 'center' }); });
...
function loadAdminPlans(){db.ref("plans").on("value",e=>{plansTableBody.innerHTML="",e.exists()&&e.forEach(child=>{const t=child.val(),a=plansTableBody.insertRow();a.innerHTML=`<td>${t.name||"N/A"}</td><td>₹${t.price||"0"}</td><td>${t.description||"N/A"}</td><td><button class="btn btn-primary" onclick="editPlan('${child.key}')">Edit</button><button class="btn btn-outline" onclick="deletePlan('${child.key}')">Delete</button></td>`})})}
