...
const UPI_NAME = 'MR SHANKAR MEHER';
...
// *** UPDATED FUNCTION: Videos/Notes open in a new tab, but Photos open inside the modal ***
function openContentDisplayModal(url, type, title) {
    ...
    // Helper function to show a message inside the modal when content opens externally
    ...
    // If it's a video, note, or a quiz with a URL, open it in a new tab for performance.
    if (type === 'video' || type === 'note' || (type === 'quiz' && url)) {
        window.open(url, '_blank', 'noopener,noreferrer');
        showNewTabMessage(url); // Show the helper message in the modal
    } 
    // If it's explicitly a photo OR the URL looks like an image, show it inside the modal.
    else if (type === 'photo' || /\.(jpe?g|png|gif|webp|svg)$/i.test(url)) {
        contentIframeWrapper.innerHTML = `<img src="${url}" alt="${title}">`;
    } 
    // For any other case, safely default to opening in a new tab.
    else {
        window.open(url, '_blank', 'noopener,noreferrer');
        showNewTabMessage(url);
    }

    // Finally, always show the modal (either with the image or the "new tab" message).
    contentDisplayModal.style.display = 'flex';
}
async function openQuizModal(quizId) {
    ...
    const quizQuestionContainer = document.getElementById('quizQuestionContainer'); // Define here
